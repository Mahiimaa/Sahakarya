...
const UserProfileInfoPage = () => {
  ...
  <div className="flex-grow flex items-center justify-center">
          ...
          {selectedServices.length > 0 ? (
              <ul className="list-disc list-inside">
                {selectedServices.map((service, index) => (
                  <li key={service._id || index} className="text-body hover:text-p" onClick={() => handleServiceClick(service)}>{service.serviceName} </li>
                ))}
            </ul>
          ) : (
    ...
    {showModal && (
        <div className="fixed inset-0 bg-dark-grey bg-opacity-50 flex items-center justify-center" onClick={closeModal}>
          <div className="bg-white p-6 rounded-md w-96 relative" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
            <button className="absolute right-3 text-xl" onClick={closeModal}>
               <IoClose size={24} />
            </button>
